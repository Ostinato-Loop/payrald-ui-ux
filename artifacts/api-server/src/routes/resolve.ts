...
import { z } from "zod";
import { lookupIdentity } from "@workspace/alia";

const router: IRouter = Router();

const ResolveIdentityBody = z.object({ alias: z.string().min(1) });

router.post("/resolve", async (req, res) => {
  const body = ResolveIdentityBody.safeParse(req.body);
  if (!body.success) return res.status(400).json({ error: "alias is required" });
  const { alias } = body.data;

  const aliaResult = await lookupIdentity(alias);
  if (aliaResult) return res.json(aliaResult);
